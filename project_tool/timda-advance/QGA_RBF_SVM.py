...
# 2.3 计算适应度函数值
    ...
    def fitness(self, population_Binary):
        ...
        for i in range(len(population_Binary)):
            ...
            for j in range(len(population_Binary[i])):
                ...
                for l in range(len(population_Binary[i][j])):
                    total += population_Binary[i][j][l] * \
                        math.pow(2, l)  # 计算二进制对应的十进制数值
        ...
        for l in range(len(parameters[0])):
            rbf_svm = svm.SVC(
                kernel='rbf', C=parameters[0][l], gamma=parameters[1][l])
...
# 2.4 变异
    def mutation(self, population_Angle_crossover, population_Angle, best_parameter_Binary, best_fitness):
        ...
        '''
        # 1.求出交叉后的适应度函数值列表
        population_Q_crossover = self.population_Q(
            population_Angle_crossover)  # 交叉后的种群量子系数列表
        population_Binary_crossover = self.translation(
            population_Q_crossover)  # 交叉后的种群二进制数列表
        ...
        for i in range(self.chromosome_num):
            for j in range(self.population_size):
                for m in range(self.chromosome_length):
                    population_Angle[i][j][m] = population_Angle[i][j][m] + \
                        Rotation_Angle[i][j][m]
...
# 2.6 主函数
    def main(self):
        ...
        for i in range(self.iter_num):
            ...
            print('iteration is :', i+1, ';Best parameters:',
                  best_parameter, ';Best fitness', best_fitness)
            results.append(best_fitness)

            # 全干扰交叉
            population_Angle_crossover = self.crossover(population_Angle)
            # 量子旋转变异
            population_Angle = self.mutation(
                population_Angle_crossover, population_Angle, current_parameter_Binary, current_fitness)
        # 结果展示
        results.sort()
        self.plot(results)
...
if __name__ == '__main__':
    ...
    deta = 0.1 * np.pi
    print('----------------3.QGA_RBF_SVM-----------------')
    qga = QGA(population_size, chromosome_num, chromosome_length,
              max_value, min_value, iter_num, deta)
    qga.main()
